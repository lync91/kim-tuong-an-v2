...
function TaoPhieu() {
  ...
  const [listTenKhach, setListTenKhach] = useState([]);
  const calc = () => {};
  const genKey = () => {
    getLastId().then((e) => {
  ...
  useEffect(() => {
    defData = new Camdo();
    (async () => {
      const _listTenKhach = await getListTenKhach();
      console.log(_listTenKhach.map((e:any) => e.tenkhach));
      setListTenKhach(_listTenKhach.map((e:any) => {
        return {value: e.tenkhach}
      }));
      console.log(options);
            ...
            >
              ...
              <Form.Item label="Trọng lượng">
                <Form.Item
                  name="tongtrongluong"
                  rules={[{ required: true }]}
                  style={{ display: "inline-block", width: "calc(32% - 4px)" }}
                  className={
