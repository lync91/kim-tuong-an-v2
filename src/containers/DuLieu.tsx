...
import { chi, vnd, lettersToNumber} from "../utils/tools";
...
export default function DuLieu() {
  ...
  const [data, setData] = useState(defPreviewData);
  ...
  const getData = async () => {
    const res: any[] = await ipcRenderer.invoke('getDataDotu');
    setData(res);
  }
    ...
    <>
      ...
      >
        ...
        <div hidden={step !== 1} style={{ marginBottom: 15 }}>
          <ReactDataGrid
            columns={columns}
            rowGetter={(i: number) => previewData[i]}
